...
The graphics/audio and Grand Prix integration round is pinned separately: `curl -fsSL https://raw.githubusercontent.com/matejbudzel/dosbox-armv6-perf-test/v0.2.2/run-integration.sh | sh`. Start it on the active `tty1` console. It benchmarks a deterministic mode-13h/PC-speaker COM workload, then launches the authoritative `GPEGA.EXE` directly with `machine=ega`, 8 MB memory, Sound Blaster Pro emulation, and the pi-286-games baseline `cycles=fixed 3000`. It runs each variant for 60 seconds. The Grand Prix soak reports process CPU time and preserved logs; it is deliberately not presented as a speed ratio. Set `GP_CYCLES=max` only for a saturation soak.
